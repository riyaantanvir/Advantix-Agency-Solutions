...
// ── Bengali TTS proxy ─────────────────────────────────────────────────────────
// Proxies text through Google Translate TTS for Bengali audio.
// This is used as a fallback when the user's device has no Bengali TTS voice.
router.get("/tools/tts", requireToolUser, async (req, res) => {
  try {
    ...
    if (text.length > 200) { res.status(400).json({ error: "text too long (max 200 chars)" }); return; }

    const { getAudioUrl } = await import("google-tts-api");
    const url = getAudioUrl(text, { lang, slow: false, host: "https://translate.google.com", timeout: 10000 });
