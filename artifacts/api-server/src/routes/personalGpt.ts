...
router.put("/admin/personal-gpt/settings", requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (body.telegramChatId !== undefined) {
      ...
      } else if (typeof body.telegramChatId === "string" && /^-?\d{3,20}$/.test(body.telegramChatId.trim())) {
        patch.telegramChatId = body.telegramChatId.trim();
      } else {
        res.status(400).json({ error: "Invalid Telegram chat ID (must be numeric)" });
        return;
      }
    }
...
router.post("/admin/personal-gpt/chat", requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await runPersonalGptTurn(message, "web");
    res.json(result);
  } catch (err) {
