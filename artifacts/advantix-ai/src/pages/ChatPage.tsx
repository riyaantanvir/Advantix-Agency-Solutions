...
export default function ChatPage() {
  ...
  async function saveEditMemory() {
    ...
    const res = await fetch(`/api/ai/memories/${editingMemory.id}`, {
      method: "PUT", credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content: editingMemory.content.trim() }),
    });
    if (res.ok) {
  ...
  async function deleteMemory(id: number) {
    await fetch(`/api/ai/memories/${id}`, { method: "DELETE", credentials: "include" });
    setMemories(prev => prev.filter(m => m.id !== id));
  }

  async function clearAllMemories() {
    if (!confirm("Delete all memories? The AI will no longer remember anything about you.")) return;
    await Promise.all(memories.map(m =>
      fetch(`/api/ai/memories/${m.id}`, { method: "DELETE", credentials: "include" })
    ));
    setMemories([]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden flex flex-col max-h-[80vh]">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => { setAddingMemory(true); setNewMemory(""); }}
                ...
                <p className="text-[11px] text-muted-foreground mb-4">
                  The AI remembers these facts about you and uses them to personalize every response. Auto-memories are saved from your conversations.
                </p>
                ...
                {addingMemory && (
                  <div className="flex gap-2 mb-3">
                    <input
                      ...
                      onKeyDown={e => { if (e.key === "Enter") addMemory(); if (e.key === "Escape") setAddingMemory(false); }}
                      placeholder="e.g. My name is Tanvir, I work as a developer..."
                      className="flex-1 h-9 px-3 rounded-lg border border-border/50 bg-background text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-primary/50 transition-colors"
                      autoFocus
                    />
                    <button
                      onClick={addMemory}
                      disabled={!newMemory.trim()}
                      className="w-9 h-9 rounded-lg bg-primary flex items-center justify-center text-primary-foreground hover:opacity-90 disabled:opacity-40 transition-opacity"
                    >
                      <Check className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => setAddingMemory(false)}
                      className="w-9 h-9 rounded-lg border border-border/50 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ...
                {memories.length === 0 && !addingMemory ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-[11px] text-muted-foreground/40 mt-1">
                      Chat with the AI and it will automatically remember things about you.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {memories.map(m => (
                      <div
                        key={m.id}
                        className="group flex items-start gap-2 px-3 py-2.5 rounded-lg border border-border/40 bg-background/50 hover:border-border/70 transition-colors"
                      >
                        <div className="flex-1 min-w-0">
                          {editingMemory?.id === m.id ? (
                            <div className="flex gap-2">
                              <input
                                ...
                                onKeyDown={e => { if (e.key === "Enter") saveEditMemory(); if (e.key === "Escape") setEditingMemory(null); }}
                                className="flex-1 text-xs bg-transparent border-b border-primary/50 outline-none text-foreground py-0.5"
                                autoFocus
                              />
                              <button onClick={saveEditMemory} className="text-primary hover:opacity-80 shrink-0">
                                <Check className="w-3.5 h-3.5" />
                              </button>
                              <button onClick={() => setEditingMemory(null)} className="text-muted-foreground hover:text-foreground shrink-0">
                                <X className="w-3.5 h-3.5" />
                              </button>
                            </div>
                          ) : (
                            <p className="text-xs text-foreground/90 leading-relaxed">{m.content}</p>
                          )}
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                              ...
                              {m.source === "auto" ? "Auto-saved" : "Manual"}
                            </span>
                          </div>
                        </div>
                        {editingMemory?.id !== m.id && (
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                            <button
                              ...
                              className="w-6 h-6 flex items-center justify-center rounded hover:bg-sidebar-accent text-muted-foreground hover:text-foreground transition-colors"
                            >
                              <Pencil className="w-3 h-3" />
                            </button>
                            <button
                              onClick={() => deleteMemory(m.id)}
                              className="w-6 h-6 flex items-center justify-center rounded hover:bg-destructive/15 text-muted-foreground hover:text-destructive transition-colors"
                            >
                              <Trash2 className="w-3 h-3" />
                            </button>
                          </div>
            ...
            <div className="px-6 py-4 border-t border-border/50 bg-background/50 shrink-0">
              <button
                onClick={() => setSettingsOpen(false)}
                className="w-full py-2 text-sm font-medium rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity"
              >
                Done
              </button>
            </div>
